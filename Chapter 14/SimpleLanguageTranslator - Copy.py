...
def run_translator():
    ...
    french_prompt = translate(text_to_translate='Press Enter and then ask your question in English', model='en-fr')
    ...
    french = translate(text_to_translate=english, model='en-fr')
    print('French:', french)

    # Step 4: Synthesize the Spanish text into Spanish speech
    text_to_speech(text_to_speak=french, voice_to_use='es-US_SofiaVoice',
        file_name='french.wav')

    # Step 5: Play the Spanish audio file
    play_audio(file_name='french.wav')

    # Step 6: Prompt for then record Spanish speech into an audio file
    input('Press Enter then speak the French answer\n' + french_prompt)
    record_audio('frenchresponse.wav')

    # Step 7: Transcribe the Spanish speech to Spanish text
    french = speech_to_text(
        file_name='frenchresponse.wav', model_id='fr-FR_BroadbandModel')
    print('French response:', french)

    # Step 8: Translate the Spanish text into English text
    english = translate(text_to_translate=french, model='fr-en')
